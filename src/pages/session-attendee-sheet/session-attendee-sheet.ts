  ...
  setFields() {
    ...
    this.session.SessionAttendees.forEach(attendee => {
      ...
      for (var checkInTime of attendee.checkInTimes) {
        ...
        //checkInTime.CheckInTime = new Date(checkInTime.CheckInTime);
        ...
        //console.log(moment(checkInTime.CheckInTime));

        checkInTime.CheckInTime = checkInTime.CheckInTime.substring(10);

      }

      attendee.checkInTimes = attendee.checkInTimes.sort(function (a, b) {
        let dateA: any = new Date(a.CheckInTime), dateB: any = new Date(b.CheckInTime);
        return dateA - dateB;
      });
